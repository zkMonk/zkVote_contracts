...
async function printERC20Balance({ address, name, tokenAddress }) {
  const erc20ContractJson = require("./artifacts/contracts/ERC20Mock.json");
  erc20 = tokenAddress
...
function createDeposit({ nullifier, secret, votingId }) {
  ...
  deposit.preimage = Buffer.concat([
    ...
    deposit.votingId.leInt2Buff(31),
  ]);
...
async function setTornado() {
  console.log(
    `Setting Tornado address ${tornado.options.address} to voting contract ${voting.options.address}`
  );
  await voting.methods
    .setTornado(tornado.options.address)
...
async function createVoting(optionsNumber, nominationValue) {
  ...
  await voting.methods
    .createVoting(optionsNumber, nominationValue)
    .send({ from: senderAccount, gas: 2e6 });
  return votingId;
}
...
async function addVotes(votingId, address, votes) {
  await voting.methods
    .addVotes(votingId, address, votes)
    .send({ from: senderAccount, gas: 2e6 });
}
...
async function getBallot({ votingId, address }) {
  ...
  await tornado.methods
    .getBallot(toHex(deposit.commitment), votingId)
    .send({ from: address ? address : senderAccount, gas: 2e6 })
...
async function generateMerkleProof(deposit) {
  ...
  const events = await tornado.getPastEvents("Ballot", {
    fromBlock: 0,
    toBlock: "latest",
  });
...
function parseNote(noteString) {
  const noteRegex =
    /tornado-(?<voteId>\d+)-(?<netId>\d+)-0x(?<note>[0-9a-fA-F]{186})/g;
...
async function init({ rpc, noteNetId }) {
  ...
  contractJson = require("./artifacts/contracts/VotingTornado.sol/VotingTornado.json");
  ...
  votingContractJson = require("./artifacts/contracts/Voting.sol/Voting.json");
...
async function main() {
  ...
  program
    ...
    .action(async () => {
      ...
      console.log(
        "Active address:",
        await getActiveAddress(program.address || "")
      );
    });
